...
| `walmart_get_taxonomy` | Full tree is ~2MB. Pass `category: "cameras"` (substring match, also matches productTypeName leaves) to get only matching subtrees plus a `matchedProductTypes` list. |
| All tools (0.5.10) | Every tool carries MCP annotations (readOnly/destructive/idempotent/openWorld hints) and mirrors object results into `structuredContent`. List tools (orders/items/returns) attach a uniform `pagination` block alongside their native shapes. |
